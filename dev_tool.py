...
def parse_data(data, match_packages, centered_labels):
    path = []
    ratio = []
...
def sub_graphs(plot_data, match_packages):
    plot_datas = []
...
def create_plots(plot_datas, sort, sort_by_ratio, ascending, non_interactive, out_filename, complete):
    plots = []
...
def create_grid(plots, complete):
    if complete:
        grid = [
            [plots[0]],
